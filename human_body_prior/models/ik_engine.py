...
# next line already out of date, due to the update of 'body_visualizer'
...
def ik_fit(optimizer, source_kpts_model, static_vars, vp_model, extra_params={}, on_step=None, gstep=0):
    ...
    def fit(weights, free_vars):
        ...
        if on_step is not None:
            on_step(opt_objs, c2c(res['body'].v), c2c(res['source_kpts']), fit.gstep)# vertices
...
class IK_Engine(nn.Module):
    ...
    def __init__(self,
                 ...
                 verbosity: int = 1,
                 logger=None,
                 device=torch.device('cuda', index=4)
                 ):
        '''
    ...
    def forward(self, source_kpts, target_kpts, initial_body_params={}):
        ...
        if self.optimizer_args['type'].upper() == 'LBFGS':
            optimizer = torch.optim.LBFGS(list(v for k,v in free_vars.items() if k in ['trans', 'poZ_body', 'root_orient', 'betas']),#list(free_vars.values()) list(v for k,v in free_vars.items() if k in ['trans', 'poZ_body', 'root_orient'])
            ...
            optimizer = AdamInClosure(list(v for k,v in free_vars.items() if k in ['trans', 'poZ_body', 'root_orient', 'betas']),# list(free_vars.values())
